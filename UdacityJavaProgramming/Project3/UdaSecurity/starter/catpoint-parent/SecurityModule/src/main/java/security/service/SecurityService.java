...
//        securityRepository.updateSensor(sensor);
    ...
    void changeStatus(Set<Sensor> sensors){
        Iterator itr = sensors.iterator();
        while (itr.hasNext()){
            Sensor sen = (Sensor) itr.next();
            changeSensorActivationStatus(sen, Boolean.valueOf(Boolean.FALSE));
        }
    }
}
